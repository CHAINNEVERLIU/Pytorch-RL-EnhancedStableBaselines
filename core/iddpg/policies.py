...
from core.maddpg.policies import Actor


class ContinuousCritic(MultiAgentBaseModel):
    """
    IDDPG算法的Critic网络，仅用局部观测和动作进行评价
    连续动作空间的Critic网络，用于IDDPG算法。
    ...
    def __init__(
        ...
        for agent_id in range(self.n_agents):
            ...
            action_dim = get_action_dim(self.action_space_list[agent_id])
            for idx in range(n_critics):
                # 创建Q网络，输入是所有智能体的联合特征和动作
                q_net_list = create_mlp(features_dim_list[agent_id] + action_dim, 1, net_arch[agent_id], activation_fn)
    ...
    def forward(self, obs: th.Tensor, actions: th.Tensor) -> list[tuple[th.Tensor, ...]]:
        ...
        for agent_id in range(self.n_agents):
            ...
            agent_action = self._agent_action_tensor_extract(agent_id, actions)
            qvalue_input = th.cat((agent_features, agent_action), dim=-1)
...
class IDDPGPolicy(MultiAgentBasePolicy):
    """
    独立多智能体深度确定性策略梯度 (IDDPG) 策略类

    IDDPG算法的特点是:
    1. 每个智能体基于自己的局部观察选择动作(去中心化执行)
    2. Critic评估基于局部状态和动作(去中心化训练)
    3. 使用目标网络进行稳定学习
    ...
    def _agent_predict(self, agent_id: int, agent_observation: PyTorchObs, deterministic: bool = False) -> th.Tensor:
        ...
        :return: 智能体的动作
        """
        # 注意：IDDPG中deterministic参数被忽略，预测总是确定性的
