...
from core.common.logger import Logger, configure
from core.common.type_aliases import GymEnv, Schedule, TensorDict, TrainFreq, TrainFrequencyUnit


def to_numpy(data):
    """确保数据是 numpy 格式"""
    if isinstance(data, th.Tensor):
        return data.detach().cpu().numpy()  # 确保转换为 numpy
    return np.array(data)  # 其他情况直接转换为 numpy 数组
