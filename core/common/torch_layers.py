...
from core.common.preprocessing import get_flattened_obs_dim, is_image_space
from core.common.type_aliases import TensorDict
from core.common.utils import get_device
...
def get_multi_agent_actor_critic_arch(net_arch: Union[list[list[int]], list[dict[str, list[int]]]])\
        -> tuple[list[list[int]], list[list[int]]]:
    """
    Get the actor and critic network architectures for off-policy multiagent actor-critic algorithms (SAC, TD3, DDPG).
    :param net_arch: The specification of the actor and critic networks.
        See above for details on its formatting.
    :return: The network architectures for the actor and the critic
    """
    actor_arch, critic_arch = [[] for _ in range(len(net_arch))], [[] for _ in range(len(net_arch))]
    for agent_id in range(len(net_arch)):
        if isinstance(net_arch[agent_id], list):
            actor_arch[agent_id], critic_arch[agent_id] = net_arch[agent_id], net_arch[agent_id]
        else:
            assert isinstance(net_arch[agent_id], dict), "Error: the net_arch can only contain be a list of ints or a dict"
            assert "pi" in net_arch[agent_id], "Error: no key 'pi' was provided in net_arch for the actor network"
            assert "qf" in net_arch[agent_id], "Error: no key 'qf' was provided in net_arch for the critic network"
            actor_arch[agent_id], critic_arch[agent_id] = net_arch[agent_id]["pi"], net_arch[agent_id]["qf"]
    return actor_arch, critic_arch
