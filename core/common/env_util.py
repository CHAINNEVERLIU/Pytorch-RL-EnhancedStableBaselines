...
from core.common.atari_wrappers import AtariWrapper
from core.common.monitor import Monitor
from core.common.vec_env import DummyVecEnv, SubprocVecEnv, VecEnv
from core.common.vec_env.patch_gym import _patch_env


def unwrap_wrapper(env: gym.Env, wrapper_class: type[gym.Wrapper]) -> Optional[gym.Wrapper]:
    """
    找到 env 是否被 wrapper_class 包裹，并返回该 Wrapper 实例
...
def is_wrapped(env: gym.Env, wrapper_class: type[gym.Wrapper]) -> bool:
    """
    通过 unwrap_wrapper(env, wrapper_class) 检查是否能找到 wrapper_class。
    如果找到（不为 None），返回 True，否则返回 False。
